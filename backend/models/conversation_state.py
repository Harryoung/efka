...
between users, the Agent, and domain experts via messaging platforms.
...
class ConversationState(Enum):
    ...
    States:
        IDLE: User is not in an active conversation, or conversation completed
        WAITING_FOR_EXPERT: Agent has contacted a domain expert, waiting for reply
        COMPLETED: Expert replied, Agent forwarded to user, FAQ updated
    """
    IDLE = "idle"
...
class ConversationContext:
    ...
    where a user asks a question, Agent contacts an expert, and waits
    for the expert's reply before responding to the user.
