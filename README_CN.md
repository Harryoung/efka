...
### 知识库 README 分层管理（渐进披露）

随着知识库增长，知了会自动管理分层的 README 结构，以保持导航效率：

**分层策略**：
- **主 README**（`knowledge_base/README.md`）：包含顶级目录概要（目录名、文件数、一句话描述）。小规模目录（<20 文件）直接展开；大规模目录链接到子目录 README。
- **子目录 README**（`knowledge_base/<dir>/README.md`）：该目录下的详细文件清单。如果子目录仍过大，继续向下拆分。

**自动触发条件**：
- 当任一目录达到 ≥50 个文件时
- 当主 README 超过 200 行时
- 管理员 Agent 会根据实际内容复杂度自主决策

**设计原则**：
- README 是导航索引，不是百科全书
- 每层 README 应能在约 3 秒内完成扫描
- 用户 Agent 只需知道「去哪找」，不需要知道「所有细节」

这种渐进披露确保无论知识库规模多大，都能保持高效导航，同时让树状搜索策略保持高性能。
