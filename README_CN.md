...
| 飞书 | 🔜 规划中 | 事件订阅 + 消息 API |
| 钉钉 | 🔜 规划中 | 机器人回调 + 消息推送 |
| Slack | 🔜 规划中 | Slack App + Events API |

每个平台都将实现 `BaseChannelAdapter` 接口。详见[渠道开发指南](docs/CHANNELS.md)。

### 企业级权限控制系统

**目标**：为多租户企业部署提供细粒度的文件级权限控制。

**核心设计**：利用 Claude Agent SDK 的 `PreToolUse` Hook，在文件访问前拦截 `Read` / `Glob` / `Grep` 工具调用并进行权限检查。

```
┌─────────────────────────────────────────────────────────┐
│  PreToolUse Hook（权限网关）                             │
│  ┌─────────────────────────────────────────────────┐    │
│  │  1. 从工具输入中提取文件路径                      │    │
│  │  2. 查询权限服务（用户 → ACL）                   │    │
│  │  3. 允许 / 拒绝 / 过滤（部分访问）               │    │
...
**核心特性**：
- **基于路径的 ACL**：在目录或文件级别定义允许/拒绝规则
- **基于角色的访问控制（RBAC）**：将用户映射到角色，角色映射到权限
- **IM 感知身份识别**：自动从企业微信 / 飞书 / 钉钉上下文解析用户身份
- **审计追踪**：记录所有文件访问尝试，满足合规需求
